...
describe('lib', () => {
  ...
  it('should recursively flattern an array input', () => {
    ...
    expect(lib([ 'a', false, 'b', [ 'c', 0, 'd' ], 'e' ])).to.equal('a b c d e');
  });
  ...
  it('should not output unnecessary white spaces', () => {
    ...
    expect(lib(' a ', ' b ', ' ', ' c ')).to.equal('a b c');
    expect(lib(` a ${space}${formFeed}${lineFeed}${carriageReturn}${hTab}${vTab} `, ' b ', `${space}${formFeed}${lineFeed}${carriageReturn}${hTab}${vTab}`, ' c ')).to.equal('a b c');
